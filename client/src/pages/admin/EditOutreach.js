...
function EditTeam() {
  ...
  useEffect(() => {
    try {
      axios.get(`http://localhost:8080/outreach`).then((res) => {
        setData(res.data);
      });
    } catch (err) {
      console.log(err);
  ...
  const addRecord = () => {
    axios.post(`http://localhost:8080/outreach`, {
      title: "Fill in here",
      description: "Fill in here",
      image: "Fill in here",
      time: "Fill in here",
      location: "Fill in here",
      inCharge: "Fill in here",
    });
  };
  const saveRecord = () => {
    console.log(record.body);
    axios.put(`http://localhost:8080/outreach`, record);
  };
  const deleteRecord = () => {
    axios.delete(`http://localhost:8080/outreach`, { data: record });
  };
  ...
  return (
    <>
      ...
      <div className="main-container">
        <br />
        <Title style={{ textAlign: "center" }}>Edit Outreach Page</Title>
          ...
          >
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Please input the In-Charge!",
                },
              ]}
            >
              {/*Need to change default*/}
              <Input
                name="inCharge"
                defaultValue={record.inCharge ? record.inCharge : ""}
              />
            </Form.Item>
          </Form>
        </Modal>
